...
def island(cells, boro="Manhattan"):
    """Open the water around Manhattan.

    Manhattan really is an island, but at 44 columns the Harlem and East
    Rivers are narrower than one block, so the rasteriser fuses it to the
    Bronx and Queens. Dropping the blocks on either side of those seams
    puts the rivers back.

    This is a deliberate liberty: those blocks are real city that the map no
    longer draws. It is a decorative map — the shading is the sun's position,
    not data — so trading a block of shoreline for a legible Manhattan is a
    fair trade. It would not be, if anything were ever measured off it.
    """
    out = {}
    for (i, j), b in cells.items():
        ...
        for di, dj in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            ...
            if nb is not None and nb != b and boro in (b, nb):
                seam = True
                break
        if not seam:
            out[(i, j)] = b
