...
**Status, 3 September 2026.** The recommendations below were implemented on this branch the day after the audit, with kit prices (headline 10, P1 item 14's kit anchors, decision D26) deliberately left out. Implemented: P0 items 1 to 7 (hosts, `intersects`, un-mirrored meshes, degraded neighbour path, awaited neighbour promise, public-copy corrections, provenance notice, privacy: BBL/BIN and rounded coordinates instead of the address); P1 items 8 to 16 (self-consumption with an occupancy toggle, the PVWatts table with a clipping table, storey-based placement, NSRDB-weighted sweep with per-component ground light and two days per month, PVWatts parameters `gcr 0.01` / `losses 9` / tilt-dependent soiling / `use_wf_albedo` / inverter-based `dc_ac_ratio`, three-tier neighbour queries, constants, headless horizon model, direct sun hours and floor/direction correction in the UI); P2 items 17 to 22 (mount-type slab band, street trees, own-facade edges and a setback flag, inverter replacement and today's-dollars total, canonical canyons in place of recalibrated bands, recorded PVWatts fixtures); P3 item 25 (an uncertainty budget on the page). Still open: P3 items 23 and 24 (a PVWatts proxy and 8,760-hour weights per address), kit anchors, and field validation. The self-consumption grid was extended to 1,296 runs (`docs/data/self-consumption-sim-2026-09-03.csv`) so the curves cover ratios up to 1.7. `docs/modeling-decisions.md` records the new status of every decision.
...
7. **Savings assume 100% self-consumption.** The methodology's "production is well below consumption so there is no excess export" conflates annual totals with instantaneous power. An hourly simulation (the DOE residential load profile for Central Park, reduced to an apartment archetype and scaled to the bill, against PVWatts hourly output) puts on-site use at 92% for the default case (800 W vertical south, $140 bill), 85% at Con Ed's typical 280 kWh/month, 73% at 200 kWh/month, and 63 to 69% when nobody is home on weekdays; a 1,600 W array is used only 49 to 69%. Under the SUNNY Act exported energy is uncompensated. The default savings figure is therefore 8% high (payback 5.2 → 5.7 years); for a typical Con Ed apartment that is empty on weekdays it is 33% high (payback 7.8 years). Section 4.4 gives the recommended fix.
8. **The 3D scene draws every building mirrored north-south.** `_createBuildingMesh` builds each footprint with `shape.moveTo(x, z)` and the `rotateX(−π/2)` extrusion maps it to world −z (verified against the exact three.js r128 build the page loads). The energy model reads the local coordinates and is unaffected, but the sun, the sun arc, the balcony marker, the horizon profile and the heatmap use the unmirrored frame, so a southern neighbour that the model correctly paints red is drawn north of the target with its rendered shadow falling away from the balcony. One-character fix.
9. **Several public constants are mis-sourced or stale.** The CO2 factor 0.89 lb/kWh is eGRID2022 (885.2 lb/MWh), not eGRID2023 as labelled (864.5 average, 976.9 non-baseload; verified in EPA's Rev 2 tables). The degradation citation `fy24osti/87524.pdf` is a community-solar report, not a degradation review (verified from the PDF title page; the intended references are NREL/TP-5K00-88769 and Jordan et al. 2022). The "48 lb CO2 per tree" figure is Arbor Day/USDA, not EPA (EPA: 0.060 t per urban tree-year, about 133 lb). The vehicle factor description ("0.906 lbs/mile rounded" to 0.89) is wrong on both ends (EPA: 0.866 to 0.882 lb/mile), and because 0.89 equals the CO2 factor, "miles not driven" always equals annual kWh. Con Ed's SC-1 customer charge is $21.00 from 1 February 2026, not $20. "56% above the U.S. average" is New York State vs US; Con Ed at 34¢ is roughly double the 2025 US average of 17.3¢.
10. **Kit anchors do not hold for New York.** Bright Saver's prices are verified but it cannot ship to New York (the SUNNY Act requires listing as a complete plug-in system, which no kit has yet). Craftstrom's 800 W kit lists at $2,031 to $2,229, not $1,327 to $1,600. EcoFlow PowerStream and Anker SOLIX RS40P are EU products; the US EcoFlow STREAM is Utah-only. The mid-tier $1,200 is still plausible as a forward estimate but the named anchors are not.
11. **Documented behaviour that is not implemented.** The "hours of direct sun" figure (methodology §2.1) is computed by `ShadowModel.directSunHours` but never displayed; the building info panel that would show it, along with the derived floor and facing, is hidden by `#bldgInfoPanel { display: none !important; }`. The manual entry panel is described as the keyboard route to a result but only opens when the 3D path fails. Geocoding failures are console-only.
...
- **Self-consumption.** Every kWh exported is worth zero: the Act makes exports uncompensated unless the owner voluntarily enters a net-metering agreement (which would bring back the interconnection process the Act removes), and Con Ed's AMI meters register delivered and received energy on separate channels, so exports are neither billed as consumption nor credited. The hourly simulation in section 4.4 puts on-site use at 92% for the default case, 85% for a typical Con Ed apartment, and 63 to 69% when nobody is home on weekdays. Utah's HB 340 (2025), the US precedent the SUNNY Act follows, has the same 1,200 W cap and the same exemption from net metering.
- **Sales tax.** New York exempts the retail sale and installation of "residential solar energy systems equipment" from the 4% state sales tax and the 3/8% MCTD tax (Tax Law §1115(ee), TSB-M-05(11)S), and New York City is listed in Publication 718-S among the jurisdictions that also waive the local tax. The statutory definition ("an arrangement or combination of components installed in a residence that utilizes solar radiation to produce ... electricity", explicitly including apartments and co-ops) plausibly covers a plug-in kit, but purchasers must present Form ST-121 and online vendors may not apply it; treat kit prices as tax-inclusive until a vendor confirms.
...
|---|---|---|---|
| 8 | Self-consumption factor on savings (see 4.4) with a "someone home in the day" toggle; battery shown as information, not as a default | M | default savings −8%; typical Con Ed apartment −15%; weekday-empty apartment −31 to −33%; 1,600 W arrays −31 to −51% |
...
**Why the current assumption fails.** Annual production below annual consumption does not prevent export. Export happens in any hour when the panel's output exceeds the apartment's load, and the default apartment's average load from 09:00 to 14:00 is 420 to 450 W against an average vertical-south output of 250 to 270 W with peaks of 500 to 600 W. For a vertical south panel most of the export happens in winter, when the panel produces most and there is no air-conditioning load (January 81% on-site use, June to August 100%).

**Method.** Hourly simulation in Node: the DOE/OpenEI residential base load profile for New York Central Park (TMY3, 8,760 hours, hour-ending stamps in local standard time) reduced to an apartment archetype (interior lights, appliances, plug loads and window air-conditioning; the single-family file's electric water heater, furnace fans and exterior lights removed), scaled to a target annual consumption; against PVWatts V8 hourly AC output for six orientations (the same NSRDB TMY, local standard time, hour-beginning index; row i of the load file is matched to PVWatts index i). Occupancy variants: "someone home" adds 150 W on weekdays 09:00 to 17:00; "nobody home" caps weekday 09:00 to 17:00 load at a 120 W refrigerator-and-standby floor; both rescaled to the same annual total so only the shape changes. Battery variants: 1 and 2 kWh usable, 90% round trip, 800 W charge and discharge cap, greedy dispatch. Limitations: the two series come from different typical years, so the coincidence of hot sunny afternoons and air-conditioning is not preserved (the archetype sensitivity below bounds this); the load archetype is a scaled single-family shape, not a measured NYC apartment; a commuter-shaped synthetic profile gives 71% where the archetype gives 92%, so occupancy is the dominant uncertainty. Results: `docs/data/self-consumption-sim-2026-09-02.csv` (864 runs).

**Self-consumption fraction (share of production used on site), no battery, base occupancy:**

| System (W DC), vertical south | 2,400 kWh/yr | 3,400 kWh/yr (Con Ed typical) | 4,200 kWh/yr (default bill) | 7,000 kWh/yr |
|---|---|---|---|---|
| 400 | 96% | 100% | 100% | 100% |
| 800 | 73% | 85% | 92% | 100% |
| 1,200 | 59% | 71% | 79% | 95% |
...
Same 800 W at 4,200 kWh/yr by orientation: 35° south 88%, 60° south 88%, vertical south-west 94%, vertical east 97%, vertical west 99% (east and west panels produce when people are getting up or coming home). Occupancy, 800 W vertical south: at 3,400 kWh/yr, 85% base, 91% someone home, 67% nobody home on weekdays; at 4,200 kWh/yr, 92% / 96% / 69%.

**As a function of the production-to-consumption ratio r** (vertical south, base occupancy): r = 0.15 → 94%, 0.20 → 85%, 0.25 → 77%, 0.30 → 71%, 0.40 → 61%, 0.50 → 53%, 0.60 → 47%, 0.80 → 39%, 1.0 → 33%. Nobody home on weekdays: 0.15 → 71%, 0.20 → 63%, 0.30 → 51%, 0.50 → 37%, 1.0 → 23%. Someone home: 0.20 → 90%, 0.30 → 76%, 0.50 → 58%. A 35° panel at the same ratio scores 5 to 10 points higher (its summer-heavy output coincides with air-conditioning); at the same wattage it scores lower because it produces more.

**Effect on the default estimate** (800 W vertical south, $1,200 kit, $0.34/kWh, 673 kWh/yr):

| Household | On-site use | Savings today | Savings corrected | Simple payback today → corrected | Exported (uncompensated) |
|---|---|---|---|---|---|
| $140 bill (4,235 kWh), base | 92% | $229 | $211 | 5.2 → 5.7 yr | 51 kWh |
| $140 bill, nobody home weekdays | 69% | $229 | $159 | 5.2 → 7.5 yr | 206 kWh |
| Con Ed typical 280 kWh/mo (3,360 kWh), base | 85% | $229 | $195 | 5.2 → 6.2 yr | 101 kWh |
| Con Ed typical, nobody home weekdays | 67% | $229 | $154 | 5.2 → 7.8 yr | 220 kWh |
| 200 kWh/mo (2,400 kWh), base | 73% | $229 | $168 | 5.2 → 7.2 yr | 180 kWh |
| 35° south, $140 bill, base | 88% | $352 | $312 | 3.4 → 3.9 yr | 121 kWh |

**Battery.** At 800 W a 1 kWh battery recovers 44 to 116 kWh/yr ($15 to $40) and a 2 kWh battery 48 to 156 kWh/yr, so a $600 to $900 battery increment pays back in 15 to 55 years at NYC rates. Only an oversized array changes that: 1,600 W vertical south with 2 kWh recovers 260 to 360 kWh/yr ($88 to $123). A battery belongs in the Customize panel as information ("what a battery would add"), not as a default.

**What not to approximate.** Using a monthly-by-hour average PV profile instead of the 8,760-hour series overstates on-site use badly (87 to 100% instead of 73 to 92%) because the hourly variability of production is what drives export; an averaged load profile against hourly PV is fine (74% vs 73%). So the model needs either PVWatts hourly output or a curve calibrated from it.

**Recommendation.** Now: multiply savings by a self-consumption fraction interpolated from the ratio table above (by r = annual production / inferred annual consumption, by orientation class, with an occupancy toggle defaulting to "base"), show the exported kWh and say it earns nothing, and label the battery option with its incremental payback. Later, once PVWatts is proxied and cached (P3 item 23): compute the overlap hourly from PVWatts output against the archetype load scaled to the bill, which also enables a time-of-use view. Either way the "1:1 offset" sentence in methodology §4.6 should go.

**Literature check (incomplete).** HTW Berlin's Stecker-Solar-Simulator computes self-consumption from 41 measured one-minute household load profiles (750 to 6,230 kWh/yr) and is the reference tool for this question; German consumer guidance for 600 to 800 W kits on 2,000 to 3,500 kWh households (ratio 0.2 to 0.4) typically quotes 55 to 70% without storage, which brackets this simulation's 61 to 85% for the same ratios (the NYC apartment profile has more daytime load from air-conditioning). Those German figures are widely quoted but were not re-verified from primary sources in this audit: the audit that was to do so was interrupted, and only its simulation results survived.
...
**Reconstructed:** the self-consumption audit (section 4.4) was interrupted twice by container restarts after its simulation had run; its result files and script survived and the write-up here was reconstructed from them by the lead reviewer. Its literature comparison was not completed (see the end of 4.4).

**Not verified:** any comparison against metered NYC balcony installations (none exist). Whether `developer.nrel.gov` is reachable from residential networks with cached DNS (it should not be; the zone is undelegated). The German self-consumption figures quoted in 4.4.
