...
**Update, 3 September 2026.** The recommendations of the September audit were implemented on branch `claude/balcony-solar-methodology-2ymsuh` (kit prices excepted, by decision). Each entry below now carries its status after that change and an *Implemented* note saying what shipped and where it differs from the recommendation. Entries that were already settled are unchanged.
...
**Implemented (2026-09-03).** The hostnames moved to `developer.nlr.gov` on 2 September; the PVWatts path is now exercised in the test suite against a recorded response (`tests/pvwatts-path.test.js`), and the result names which request variant answered.
...
**Implemented (2026-09-03).** `gcr 0.01` shipped, switching off the row-to-row self-shading loss that a single panel does not have.
...
**Implemented (2026-09-03).** Option (b): `losses 9`. The page documents the decomposition (PVWatts' default bundle minus soiling and shading, 9.6% compounded, rounded to 9). No unmodelled-shading allowance is carried in `losses` now that trees and the slab are modelled (D16, D18).
...
**Implemented (2026-09-03).** Tilt-dependent arrays: `1|1|2|3|3|2|2|2|2|1|1|1` for 90° (mean 1.75%) and `2|2|3|4|4|3|3|3|3|2|2|2` for 35° to 70° (mean 2.75%). The bracketed JSON retry was dropped (always HTTP 422); the fold-into-losses retry uses the array's mean (10.6% vertical, 11.5% tilted).
...
**Implemented (2026-09-03).** `use_wf_albedo 1` is sent explicitly so the weather file's monthly albedo (0.49 in the NYC January) is used; `albedo 0.20` remains the value used when the file has none. The page says so.
...
**Implemented (2026-09-03).** Option (b): the separable formula was removed and replaced by `js/pvwatts-nyc-table.js`, a 4 × 8 PVWatts V8 grid (1 kW, `dc_ac_ratio 1.0`, the shipped parameters) with monthly shares, generated by `scripts/build-pvwatts-table.mjs` and interpolated between compass azimuths on energy. Arrays larger than their inverter take a measured clipping loss (`SolarAPI.clippingLoss`, per tilt). The extra 0.95 soiling derate is gone. A test holds the table within a few percent of a recorded live response.
...
**Implemented (2026-09-03).** Monthly shares come from the PVWatts table per tilt and azimuth (a vertical south panel: January 11.5%, June 5.0%). The hardcoded GHI curve and the Solar Resource API call were removed.
...
**Implemented (2026-09-03).** `dc_ac_ratio` is now array watts over the inverter option (800 or 1,200 W AC, in the Customize panel), minimum 1.0, so PVWatts clips an oversized array as the inverter would. `inv_eff 96.5` unchanged.
...
**Implemented (2026-09-03).** D15 and D19 shipped; the profile now also includes the visitor's own footprint and skips only sample points within 1 m of the panel.
...
**Implemented (2026-09-03).** Option (b): the ground-reflected component is `albedo × GHI × (1 − cos β)/2` and is derated by the street's horizontal sky openness, computed from the same footprints at a point 6 m in front of the balcony at 1 m height (`computeGroundOpenFraction`). Clamp floor lowered to 0.02 (D14).
...
**Implemented (2026-09-03).** Option (a): `js/irradiance-nyc.js`, a 12 × 24 table of mean DNI, DHI and monthly albedo from the NSRDB typical year (PVWatts hourly output, local standard time), generated by `scripts/build-irradiance-table.mjs`. Beam = DNI × cos θ, sky = DHI × (1 + cos β)/2. Option (b), PVWatts hourly output per address, waits on a server-side proxy (audit P3 item 23).
...
**Implemented (2026-09-03).** Two sample days per month (the 8th and 23rd), 10-minute steps, energy-weighted annual factor (Σ received / Σ assumed over the year, days-in-month weighted), clamp floor 0.02.
...
**Implemented (2026-09-03).** Panel centre at `elevation + (floor − 1) × storey + 0.8 m`, with storey = roof height / floors clamped to 2.7 to 4.5 m (3.0 m default); the clicked floor is derived from the storey height; the point sits at the click projected onto the facade (middle 90%) and 1.5 m outside it. The manual path uses the facade midpoint. The Customize panel can correct the floor and the direction.
...
**Implemented (2026-09-03).** The railing factor is kept (0.95 / 0.97 / 0.98 / 0.99) and now stands for the railing alone. The slab is a geometric ceiling band from a one-question mount type: overhang 0 m (hangs outside the railing, default), 0.6 m (stands on the floor), 1.5 m (back wall); h = storey − 0.8 m; no slab on the top floor. Measured on the canonical side street, floor 3 of 6, south: vertical 0.38 / 0.35 / 0.17, 35° 0.59 / 0.44 / 0.15.
...
**Implemented (2026-09-03).** `intersects()` against a WKT circle for the neighbour tiers and against a point for the BIN-less target lookup (then nearest within 30 m). Three tiers, `$limit 1500` each, merged by BIN, with truncation flagged and disclosed: everything within 200 m, buildings over 150 ft within 500 m, buildings over 350 ft within 1,500 m. The 150 ft tier stops at 500 m rather than the recommended 800 m to keep the payload small: a 46 m building at 500 m subtends about 5°, below which a south panel's winter beam is already scarce; the 350 ft tier catches the towers that matter out to 1.5 km. The neighbour promise is awaited rather than re-issued, and a failed query is treated as degraded (canonical canyon, disclosed) instead of an open sky.
...
**Implemented (2026-09-03).** Option (a): Forestry Tree Points within 80 m (up to 400, `tpstructure = 'Full'`), DBH allometry (crown base 3 m, top min(22, 4 + 0.55 DBH), radius min(8, 1.2 + 0.30 DBH)), written into leaf-season and bare transmittance masks (0.2 May to October, 0.6 otherwise) over azimuth × altitude; beam transmittance and sky openness both read the mask. Awnings, AC units and signage remain unmodelled and disclosed.
...
**Implemented (2026-09-03).** Option (a) whenever footprints exist: `js/shade-geometry.js` and the horizon model run headlessly (no Three.js), so the manual and no-WebGL paths give the same numbers as the 3D view. For the no-footprint case, rather than the fitted curve in (b), the visitor's description becomes a canonical canyon (some: 18 m street, 24 m opposite; dense: 18 m, 45 m; wide avenue: 30 m, 30 m; open: nothing) and the same model runs on it, so the two paths cannot drift apart. The tanh bands are gone and the shading control is inert, and marked so, on the 3D path.
...
**Implemented (2026-09-03).** The target footprint is found by BIN, then by point-in-polygon (`intersects` with a POINT), then by proximity within 30 m, with the source recorded and the proximity case disclosed; the floor count comes from PLUTO, else roof height / 10 ft, else 20, with the source disclosed; a geocoding failure shows a message and opens the manual panel; the Geoclient proxy checks the request origin and rate-limits by IP.
...
**Implemented (2026-09-03).** Rate 0.35 and customer charge 21 shipped (`js/config.js`); the page notes the June to September delivery block and cites PSC Case 25-E-0072 and EIA table 5.6.A (US average 17.3¢). The Con Ed and DPS PDF links that returned 404 were replaced.
...
**Implemented (2026-09-03).** Option (b): `js/self-consumption.js` interpolates the share of production used on site from `js/self-consumption-table.js` by r = production / inferred consumption, orientation class (t90_s, t90_sw, t90_e, t90_w, t35_s, t60_s) and occupancy (out most weekdays / someone home / nobody weekdays, a Customize control). Savings, bill offset and payback use the self-consumed energy; the result shows the exported kWh as uncompensated and what a 1 kWh battery would add. The simulation grid was extended the same day with 1,200 and 1,700 kWh consumption levels (1,296 runs, `docs/data/self-consumption-sim-2026-09-03.csv`) so the curves reach r ≈ 1.1 for vertical and 1.7 for tilted panels; beyond that the curve is extended along its last segment with a 15% floor. Option (c), hourly overlap per address, waits on the PVWatts proxy.
...
**Implemented (2026-09-03).** Nominal headline kept; one inverter replacement at 30% of kit cost in year 12 inside both payback and lifetime figures; a today's-dollars total at 2.5% inflation shown alongside; system-level degradation 0.4 / 0.6 / 0.9%/yr by tier; citations corrected to NREL/TP-5K00-88769 and Jordan et al. 2022 (NREL/CP-5K00-81314).
...
**Status:** Provisional (deliberately not changed on 2026-09-03).

**Implemented (2026-09-03).** Kit prices were left out of the September fixes by decision; the tiers stay as forward estimates and the page says the anchors are under review.
...
**Implemented (2026-09-03).** Customer charge 21 and rate 0.35 shipped; the default bill is $120 (3,394 kWh/yr, 283 kWh/month), close to Con Ed's typical apartment; the input is clamped to $20 to $800.
...
**Implemented (2026-09-03).** 0.86 lb/kWh (eGRID2023 NYCW 864.5 lb/MWh) shipped and presented as a year-one average-grid figure; the non-baseload 0.98 is stated on the page as the alternative and kept in config as `CO2_FACTOR_NONBASELOAD`.
...
**Implemented (2026-09-03).** EPA factors shipped: 132 lb CO2 per urban tree per year, 0.88 lb per vehicle mile, 19 Wh per smartphone charge; miles and kWh are no longer the same number.
...
**Implemented (2026-09-03).** The page states a term-by-term uncertainty budget and a 15 to 25% band on production for a typical apartment, with savings flagged as wider and dominated by self-consumption; the FAQ, schema, footer and `llms.txt` say the same. Field validation remains open until legal installations exist.
...
**Implemented (2026-09-03).** The PVWatts path runs against a recorded fixture; the offline table is checked against the live response; geometry, shade, self-consumption and content tests were added (`tests/run.js`), 160-odd tests in all.
...
**Implemented (2026-09-03).** The result always carries a "What this estimate used" notice naming the production source and variant, the shade source with building and tree counts, truncation, setback outliers, the floor-count source and the self-consumption share; degraded paths are marked. The overlay text no longer promises an hourly simulation before one has run; a privacy note sits under the address box and the log carries BBL/BIN and rounded coordinates instead of the address.
